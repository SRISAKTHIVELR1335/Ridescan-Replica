#!/usr/bin/env bash
# RideScan Replica — hermetic build pipeline (no Gradle / no network)
set -euo pipefail
...
if [ ! -f "$KS" ]; then
  ...
  $KEYTOOL -genkeypair -alias ridescan -keyalg RSA -keysize 2048 -validity 10950 \
    -keystore "$KS" -storepass $KSPASS -keypass $KSPASS \
    -dname "CN=RideScan Replica, OU=Engineering, O=Replica, L=Chennai, ST=Tamil Nadu, C=IN" >/dev/null 2>&1
fi
...
$JAVA -jar "$APKSIGNER" sign \
  ...
  --min-sdk-version 24 \
  --out "$ROOT/RideScan-Replica.apk" \
  "$OUT/app-unsigned.apk" 2>/dev/null

echo
echo "✔ Built: $ROOT/RideScan-Replica.apk ($(du -h "$ROOT/RideScan-Replica.apk" | cut -f1))"
